...
class SelfTrainingApp(QMainWindow):
    def __init__(self):
        ...
        self.onehot_encoder = OneHotEncoder(handle_unknown='ignore', sparse_output=False)  # Fixed parameter
    ...
    def apply_preprocessing(self):
        ...
        try:
            ...
            if self.impute_checkbox.isChecked() and np.isnan(X_processed).any():
                self.imputer = SimpleImputer(strategy='mean')
    ...
    def train_supervised_model(self):
        ...
        try:
            ...
            if self.y_true_unlabeled is not None:
    ...
    def update_visualizations(self):
        ...
        if self.y_true_unlabeled is not None and self.predicted_labels is not None:
            cm = confusion_matrix(self.y_true_unlabeled, self.predicted_labels)
    ...
    def update_comparison_tab(self):
        ...
        # Calculate metrics
        accuracy = accuracy_score(self.y_true_unlabeled, self.predicted_labels)
        precision = precision_score(self.y_true_unlabeled, self.predicted_labels, average='weighted', zero_division=0)
        recall = recall_score(self.y_true_unlabeled, self.predicted_labels, average='weighted', zero_division=0)
        f1 = f1_score(self.y_true_unlabeled, self.predicted_labels, average='weighted', zero_division=0)
        ...
        n_samples = min(100, len(self.y_true_unlabeled))
        ...
        try:
            ...
            if file_path:
                ...
                results_df = pd.DataFrame({
                    'true_label': self.y_true_unlabeled,
                    'predicted_label': self.predicted_labels,
                    'is_correct': self.y_true_unlabeled == self.predicted_labels
                })
                
                # Add decoded labels if encoder was used
                if hasattr(self.label_encoder, 'classes_'):
                    results_df['true_label_decoded'] = self.label_encoder.inverse_transform(self.y_true_unlabeled)
                    results_df['predicted_label_decoded'] = self.label_encoder.inverse_transform(self.predicted_labels)
